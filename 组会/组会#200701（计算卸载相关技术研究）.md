...
### 2.2 基于智能化
#### 2.2.1 收益模型
  - 通信收益$R_{mn}^{comm}$，式中$\alpha_{mn}$为用户的接入费用，$r_{mn}$为传输速率，$\beta_{mn}$为频谱的使用费用，$w_{mn}$为信道带宽
  $$R_{mn}^{comm}=\alpha_{mn}r_{mn}-\beta_{mn}w_{mn}$$
  - 计算收益$R_{mn}^{comp}$，式中$\phi_{mn}$为用户的计算费用，$\varphi_{mn}$为边缘云的计算费用
  $$R_{mn}^{comp}=\phi_{mn}q_{mn}-\varphi_{mn}e_{mn}$$
  - 缓存收益$R_{mn}^{cache}$，式中$\psi_{mn}$为文件缓存在边缘云的存储费用，$\gamma_{mn}$为边缘云的回传成本
  $$R_{mn}^{cache}=\psi_{mn}l_{A'_m}-\gamma_{mn}A'_{m}$$

#### 2.2.2 深度*Q*学习算法
  - *Q*学习
    - *Q*学习算法的决策过程基于*MDP*，它可以用五元组来表示
    $$\left\{s_i,a_i,P\left(s_i,a_i,s_{i+1}\right),R(s_i,a_i),Q(s_i,a_i)\right\}$$
    - 系统在状态$s_i$下，基于策略$\pi$的长期收益为$V^{\pi}(s_i)$。$\lambda(0<\lambda<1)$为**折扣因子**
